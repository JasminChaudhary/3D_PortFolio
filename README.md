...
[reactjs18-3d-portfolio](https://3-d-port-folio-umm1.vercel.app/)
...
- [Vercel](https://vercel.com/): Vercel is a cloud platform for frontend developers, providing the
  frameworks,and infrastructure to build a faster, more personalized Web.
...
you need to create EmailJS account [here](https://emailjs.com/) and set the
...
The contact form sends messages via EmailJS to the address configured in `src/constants/config.ts` under `config.html.email`.

1. Create an EmailJS account and service.
2. Create a template with variables: `form_name`, `to_name`, `from_email`, `to_email`, `message`.
3. Put your credentials in `.env` using the variables above.
4. Ensure `config.html.email` is set to your inbox.
